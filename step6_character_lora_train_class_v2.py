...
class TrainingConfig:
    # --- モデルとデータセット ---
    model_name: str = "ToPo-ToPo/rinna-japanese-gpt-neox-3.6b-lora-sft-v1" # ベースモデル
    hf_dataset_id: str = "ToPo-ToPo/ai-characters-QA"
    hf_data_file: str = "dataset-Ai.jsonl" # お手元のファイル名に合わせてください
    ...
    batch_size: int = 1           # 1回に処理するデータ数
    grad_accum_steps: int = 4     # 勾配蓄積
    ...
    # --- システム設定 ---
    device_map: str = "auto"      # Macなら"mps", NVIDIAなら"auto"
    use_float32: bool = False     # NVIDIAならFalse(fp16), Mac/CPUならTrue
...
class LoRATrainer:
    ...
    def _process_data_function(self, data_point):
        ...
        for msg in messages:
            ...
            # note: role == "system" (キャラ設定) は
            # 学習データに含めない方針のため無視します。
            # 口調(style)だけを assistant_text から学習させます。
        ...
        # Rinna形式のプロンプト構築
        # ユーザーの発言の中に文脈や指示がすべて含まれている前提です
        full_prompt = (
            f"ユーザー: {user_text}<NL>"
            f"システム: {assistant_text}"
        )
    ...
    def prepare_dataset(self):
        """データセットの読み込みと加工"""
        print(f"データセット取得中: {self.c.hf_dataset_id}")
        
        # ローカルファイル(jsonl)がある場合を想定
        # 無ければ引数の hf_dataset_id からDLする処理などに書き換えてください
        ...
        # 検証用データの分割
        train_val = dataset.train_test_split(test_size=0.1, shuffle=True, seed=42) # 10%を検証用に
